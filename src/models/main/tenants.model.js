...
exports.updateTenant = async (tenant_id, updates) => {
  ...
  const updateTenantQuery =
    "update tenants set name=$1, emirates_id=$2, phone_number=$3, email=$4, date_settle_in=$5 where tenant_id=$6;";
  const updateTenantValues = [
    tenantObj.name,
    tenantObj.emirates_id,
    tenantObj.phone_number,
    tenantObj.email,
    tenantObj.date_settle_in,
    tenant_id,
  ];
...
exports.moveToRoom = async (newRoom_id, tenant_id) => {
  ...
  if (verifyRoomVacant) {
    const updateRoomIdQuery =
      "update tenants set room_id=$1 where tenant_id=$2;";
  ...
  const checkAssignedCapacityQuery =
    "select capacity from rooms where room_id=$1";
  const assignedCapacity = await db.query(checkAssignedCapacityQuery, [
    newRoom_id,
  ]);
